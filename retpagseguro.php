...
$lnk = mysqli_connect("$host", "$user", "$passwd") or die ('Nao foi possível conectar ao MySql: ' . mysqli_error());
mysqli_select_db("$db", $lnk) or die ('Nao foi possível ao banco de dados selecionado no MySql: ' . mysqli_error());    
...
mysqli_query("INSERT into PagSeguroTransacoes SET
...
mysqli_query("UPDATE accounts SET premium_points = premium_points + '$NumItens' * 2 WHERE name = '".htmlspecialchars($accname)."'");
mysqli_query("UPDATE accounts SET backup_points = backup_points + '$NumItens' * 2 WHERE name = '".htmlspecialchars($accname)."'");
mysqli_query("UPDATE PagSeguroTransacoes SET StatusTransacao = 'Entregue' WHERE CONVERT( `PagSeguroTransacoes`.`TransacaoID` USING utf8 ) = '$TransacaoID' AND CONVERT( `PagSeguroTransacoes`.`StatusTransacao` USING utf8 ) = 'Aprovado' LIMIT 1 ;");
mysqli_query('OPTIMIZE TABLE  `pagsegurotransacoes`');
}
}
?>
